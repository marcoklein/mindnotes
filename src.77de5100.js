...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"core/events/EventHandler.ts":[function(require,module,exports) {
"use strict";
...
var SerializedEventEmitter = /*#__PURE__*/function () {
  function SerializedEventEmitter() {
    _classCallCheck(this, SerializedEventEmitter);

    this.handlers = {};
  }

  _createClass(SerializedEventEmitter, [{
    key: "registerHandler",
    value: function registerHandler(eventName, handler) {
      this.handlers[eventName] = handler;
    }
  }, {
    key: "handleSerializedEvent",
    value: function handleSerializedEvent(eventJson) {
      var data = JSON.parse(eventJson);
      this.emit(data);
    }
  }, {
    key: "emit",
    value: function emit(event) {
      var handler = this.handlers[event.name];
  ...
  return SerializedEventEmitter;
}();

exports.SerializedEventEmitter = SerializedEventEmitter;
...
var GraphLinkData = /*#__PURE__*/function () {
  function GraphLinkData(graphData, source, target) {
    _classCallCheck(this, GraphLinkData);
...
},{}],"renderer/graph/GraphNode.ts":[function(require,module,exports) {
"use strict";
...
var GraphLink_1 = require("./GraphLink");
/**
...
var GraphNode = /*#__PURE__*/function () {
   ...
   */
  function GraphNode(graphData, id, headText) {
  ...
  _createClass(GraphNode, [{
    ...
    value: function changeParent(parentId) {
      ...
      var parent = this.graphData.mindmap.getNode(parentId);

      if (parent) {
        var link = new GraphLink_1.GraphLinkData(this.graphData, this, parent);
    ...
    key: "changeText",
    value: function changeText(text, line) {
...
},{"./GraphLink":"renderer/graph/GraphLink.ts"}],"renderer/graph/GraphData.ts":[function(require,module,exports) {
"use strict";
...
var GraphData = /*#__PURE__*/function () {
  ...
  _createClass(GraphData, [{
    ...
    key: "getLinkFromNodeIds",
    value: function getLinkFromNodeIds(sourceId, targetId) {
...
},{}],"renderer/graph/MindmapView.ts":[function(require,module,exports) {
"use strict";
...
var GraphData_1 = require("./GraphData");
/**
...
var MindmapView = /*#__PURE__*/function () {
  function MindmapView() {
    var _this = this;

    _classCallCheck(this, MindmapView);

    this.ticked = function () {
      var _a, _b;

      (_a = _this.node) === null || _a === void 0 ? void 0 : _a.call(_this.updateNode);
      (_b = _this.link) === null || _b === void 0 ? void 0 : _b.call(_this.updateLink);
    };

    this.updateNode = function (node) {
      node.attr("transform", function (d) {
        return "translate(" + _this.fixna(d.x) + "," + _this.fixna(d.y) + ")";
      });
    };

    this.updateLink = function (link) {
      link.attr('x1', function (d) {
        return _this.fixna(d.source.x);
      }).attr('y1', function (d) {
        return _this.fixna(d.source.y);
      }).attr('x2', function (d) {
        return _this.fixna(d.target.x);
      }).attr('y2', function (d) {
        return _this.fixna(d.target.y);
      });
    }; // setup svg
    ...
    var width = 400;
    var height = 400;
    var svg = d3.select('#renderContainer').attr('width', width).attr('height', height);
    svg.style('background-color', 'lightblue');
    this.container = svg.append('g'); // base graph data

    this.graphData = new GraphData_1.GraphData(this); // create simulation

    this.simulation = this.createGraphLayout(this.graphData, width, height); // render links and nodes

    this.link = this.container.append('g').selectAll('link');
    this.node = this.container.append('g').selectAll('node'); // enable zooming

    svg.call(d3.zoom().scaleExtent([.1, 4]).on('zoom', function () {
      return _this.container.attr('transform', d3.event.transform);
    }));
    this.renderNodes();
    this.renderLinks();
  }
  ...
  _createClass(MindmapView, [{
    key: "addNewNode",
    value: function addNewNode(id, text) {
      var node = new GraphNode_1.GraphNode(this.graphData, id, text);
    ...
    key: "createGraphLayout",
    value: function createGraphLayout(graphData, width, height) {
      return d3.forceSimulation(graphData.getNodes()).force('charge', d3.forceManyBody().strength(-3000)).force('center', d3.forceCenter(width / 2, height / 2)).force('x', d3.forceX(width / 2).strength(1)).force('y', d3.forceY(height / 2).strength(1)).force('link', d3.forceLink(graphData.getLinks()).id(function (d) {
        return d.id;
      }).distance(100).strength(1)).on('tick', this.ticked);
    }
  }, {
    key: "renderLinks",
    value: function renderLinks() {
      ...
      this.link = this.link.data(this.graphData.getLinks(), function (l) {
        return l.id;
      });
    ...
    value: function renderNodes() {
      ...
      this.node = this.node.data(this.graphData.getNodes(), function (n) {
        return n.id;
      });
      this.node.exit().remove();
      var nodeGraphics = this.node.enter().append('g');
      nodeGraphics.append('ellipse').attr('rx', 40).attr('ry', 20).attr('fill', '#888');
      nodeGraphics.append('text').attr('text-anchor', 'middle').attr('alignment-baseline', 'central').text(function (n) {
        return n.getText();
      });
      this.node = this.node.merge(nodeGraphics); // ensure updated text

      var texts = this.node.selectAll('text');
    ...
    value: function restartSimulation() {
      ...
      this.simulation.force('link').links(this.graphData.getLinks());
...
},{"d3":"../node_modules/d3/index.js","./GraphNode":"renderer/graph/GraphNode.ts","./GraphData":"renderer/graph/GraphData.ts"}],"core/events/EventTypes.ts":[function(require,module,exports) {
"use strict";
...
var ParseEvent = function ParseEvent(name) {
  _classCallCheck(this, ParseEvent);
...
var NodeEvent = /*#__PURE__*/function (_ParseEvent) {
  _inherits(NodeEvent, _ParseEvent);

  var _super = _createSuper(NodeEvent);

  function NodeEvent(name, id) {
    var _this;

    _classCallCheck(this, NodeEvent);
...
var AddNodeEvent = /*#__PURE__*/function (_NodeEvent) {
  _inherits(AddNodeEvent, _NodeEvent);

  var _super2 = _createSuper(AddNodeEvent);

  function AddNodeEvent(id, parentId, text) {
    var _this2;

    _classCallCheck(this, AddNodeEvent);

    _this2 = _super2.call(this, AddNodeEvent.NAME, id);
    _this2.id = id;
...
exports.AddNodeEvent = AddNodeEvent;
AddNodeEvent.NAME = 'node.add';

var EditNodeEvent = /*#__PURE__*/function (_NodeEvent2) {
  _inherits(EditNodeEvent, _NodeEvent2);

  var _super3 = _createSuper(EditNodeEvent);

  function EditNodeEvent(id, parentId, text) {
    var _this3;

    _classCallCheck(this, EditNodeEvent);

    _this3 = _super3.call(this, EditNodeEvent.NAME, id);
    _this3.id = id;
  ...
  return EditNodeEvent;
}(NodeEvent);

exports.EditNodeEvent = EditNodeEvent;
EditNodeEvent.NAME = 'node.edit';

var DeleteNodeEvent = /*#__PURE__*/function (_NodeEvent3) {
  _inherits(DeleteNodeEvent, _NodeEvent3);

  var _super4 = _createSuper(DeleteNodeEvent);

  function DeleteNodeEvent(id) {
    var _this4;

    _classCallCheck(this, DeleteNodeEvent);

    _this4 = _super4.call(this, DeleteNodeEvent.NAME, id);
    _this4.id = id;
    return _this4;
  }
...
exports.DeleteNodeEvent = DeleteNodeEvent;
DeleteNodeEvent.NAME = 'node.delete';
},{}],"renderer/EventRenderer.ts":[function(require,module,exports) {
"use strict";
...
var MindmapView_1 = require("./graph/MindmapView");

var EventTypes_1 = require("../core/events/EventTypes");
/**
...
var EventRenderer = /*#__PURE__*/function () {
  function EventRenderer(eventHandler) {
    var _this = this;

    _classCallCheck(this, EventRenderer);

    this.handlers = {
      addNodeEvent: function addNodeEvent(event) {
        console.log('Received event in handler: ', event);
        var parentId = event.parentId !== undefined ? '' + event.parentId : undefined;

        _this.mindmap.addNewNode('' + event.id, event.text).changeParent(parentId);
      },
      editNodeEvent: function editNodeEvent(event) {
        console.log('Received event in handler: ', event); // find node

        var node = _this.mindmap.getNode('' + event.id);
      ...
      deleteNodeEvent: function deleteNodeEvent(event) {
        console.log('Received event in handler: ', event); // find node
  ...
  _createClass(EventRenderer, [{
    key: "initEventHandlers",
    value: function initEventHandlers() {
      this.eventHandler.registerHandler(EventTypes_1.AddNodeEvent.NAME, this.handlers.addNodeEvent);
      this.eventHandler.registerHandler(EventTypes_1.EditNodeEvent.NAME, this.handlers.editNodeEvent);
      this.eventHandler.registerHandler(EventTypes_1.DeleteNodeEvent.NAME, this.handlers.deleteNodeEvent);
    }
  }, {
    key: "initMindmap",
    value: function initMindmap() {
      var mindmap = new MindmapView_1.MindmapView();
      return mindmap;
    }
  }]);
...
},{"./graph/MindmapView":"renderer/graph/MindmapView.ts","../core/events/EventTypes":"core/events/EventTypes.ts"}],"core/logger/JustLogger.ts":[function(require,module,exports) {
"use strict";
...
var Logger = /*#__PURE__*/function () {
  ...
  _createClass(Logger, [{
    key: "logWithParams",
    value: function logWithParams(level, params) {
      console.log("".concat(this.label, " ").concat(Level[level]), this.formatLog(params));
    }
    /**
    ...
    key: "formatLog",
    value: function formatLog(params) {
      var _this = this;
      ...
      if (typeof params[0] === 'string') {
        ...
        var argIndex = 0;
        return params[0].replace(/%([a-zA-Z%])/g, function (match, format) {
          if (match === '%%') {
            // skip %
            return match;
          } // next parameter
          ...
          if (typeof formatter === 'function') {
            match = formatter.call(_this, params[argIndex]);
          }
...
var JustLogger_1 = require("./core/logger/JustLogger"); // const logger = winston.createLogger({
//     level: 'debug',
...
var attachLogger = function attachLogger(label) {
  // log.getContext().namespace = label;
  return new JustLogger_1.Logger(label); // return log.child({label});
};

exports.coreLogger = function (label) {
  return attachLogger('core.' + label);
};

exports.rendererLogger = function (label) {
  return attachLogger('renderer.' + label);
};

exports.editorLogger = function (label) {
  return attachLogger('editor.' + label);
};
},{"./core/logger/JustLogger":"core/logger/JustLogger.ts"}],"core/parser/lexer/LexemType.ts":[function(require,module,exports) {
"use strict";
/*
 * List of available lexem types.
 */

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.LEXEM_EMPTY_LINE = 'empty';
exports.LEXEM_INDENT = 'indent';
exports.LEXEM_NODE_TEXT = 'text';
exports.LEXEM_MULTILINE_INDICATOR = 'multilineIndicator';
exports.LEXEM_PROPERTY_LIST_START = 'propertyListStart';
exports.LEXEM_PROPERTY_LIST_END = 'propertyListEnd';
exports.LEXEM_PROPERTY_SEPARATOR = 'propertySeparator';
exports.LEXEM_PROPERTY_ASSIGN = 'propertyAssign';
exports.LEXEM_PROPERTY_NAME = 'propertyName';
exports.LEXEM_PROPERTY_VALUE = 'propertyValue';
},{}],"core/parser/lexer/MindNotesLexer.ts":[function(require,module,exports) {
"use strict";
...
var LexemType_1 = require("./LexemType");

exports.REGEX_LINE = /^(\s*)(.*)/;
exports.REGEX_NODE = /(\+?)(\s*)(.*)/;
/**
 * The Lexer extracts tokens, called lexemes of each line.
 */

var MindNotesLexer = /*#__PURE__*/function () {
  function MindNotesLexer() {
    _classCallCheck(this, MindNotesLexer);
  }

  _createClass(MindNotesLexer, [{
    key: "tokenizeLine",
    value: function tokenizeLine(inputLine) {
      var curStart = 0;
      var result = {
        lexems: [],
        errors: []
      ...
      if (/^(\s*)$/.test(inputLine)) {
        result.lexems.push({
          type: LexemType_1.LEXEM_EMPTY_LINE,
          start: curStart,
      ...
      var lineSplit = inputLine.split(exports.REGEX_LINE);
      var indentation = lineSplit[1];

      if (indentation.length) {
        // we have indentation
        result.lexems.push({
          type: LexemType_1.LEXEM_INDENT,
          start: curStart,
      ...
      var lineBody = lineSplit[2]; // evaluate tag if given

      if (lineBody.charAt(0) === '(') {// evaluate props list
      } else if (lineBody.charAt(0) === '#') {// evaluate comment
      } else {
        var nodeBodySplit = lineBody.split(exports.REGEX_NODE); // test for multiline tag

        var multilineIndicator = nodeBodySplit[1];

        if (multilineIndicator.length) {
          // multiline tag is present
          result.lexems.push({
            type: LexemType_1.LEXEM_MULTILINE_INDICATOR,
            start: curStart,
        ...
        curStart += nodeBodySplit[2].length; // rest is node text

        var nodeText = nodeBodySplit[3];

        if (nodeText.length) {
          result.lexems.push({
            type: LexemType_1.LEXEM_NODE_TEXT,
            start: curStart,
  ...
  return MindNotesLexer;
}();

exports.MindNotesLexer = MindNotesLexer;
},{"./LexemType":"core/parser/lexer/LexemType.ts"}],"core/parser/logger.parser.ts":[function(require,module,exports) {
"use strict";
...
exports.parserLogger = function (label) {
  return logger_1.coreLogger('parser.' + label);
};
},{"../../logger":"logger.ts"}],"core/parser/LineParser.ts":[function(require,module,exports) {
"use strict";
...
var logger_parser_1 = require("./logger.parser");

var EventTypes_1 = require("../events/EventTypes");

var log = logger_parser_1.parserLogger('line');
/**
...
var LineParser = /*#__PURE__*/function () {
  ...
  _createClass(LineParser, [{
    key: "start",
    value: function start(line, content) {
      this.started = true;
      this.parsedLine = {
        line: line,
        indentation: 0,
    ...
    value: function deleteLine() {
      var node = this.tree.nodesAtLine[this.parsedLine.line];
      log.debug('Deleting line from node "%s"', node === null || node === void 0 ? void 0 : node.headText);

      if (node) {
        if (this.getPreviousNode() !== node) {
          log.debug('Removing node'); // remove node - its the head

          this.deleteNode(node);
        } else {
          log.warn('Removing stuff from node body is not supported'); // remove something from the node
          // node.deleteLine() ?
        }
      }
    ...
    key: "pushError",
    value: function pushError(name, text) {
      if (!this.parsedLine) {
        throw this.handleNotStarted();
      }

      log.warn('Parse Error (%s) on line %i (%s): %s', name, this.parsedLine.line, this.parsedLine.content, text);
      this.parsedLine.errors.push({
        line: this.parsedLine.line,
        content: this.parsedLine.content,
        name: name,
        text: text
      });
    }
     ...
     */
    value: function getExistingNode() {
    ...
    key: "getPrevIndent",
    value: function getPrevIndent() {
      return this.tree.prevIndent(this.parsedLine.line);
    }
  }, {
    key: "editNode",
    value: function editNode(node, text) {
      var _a; //log.verbose('Node at line already exists with text %s', text);
      // fire edit


      node.headText = text;
      var event = new EventTypes_1.EditNodeEvent(node.id, (_a = node.parent) === null || _a === void 0 ? void 0 : _a.id, text);
      this.parsedLine.events.push(event);
    }
  }, {
    key: "deleteNode",
    value: function deleteNode(node) {
      log.debug('Removing node "%s"', node.headText); // only the head of a node can be removed

      var previousNode = this.getPreviousNode();
      var curLine = this.parsedLine.line; // FIXME handle preceding text that is still part of the deleted node head

      do {
        this.tree.nodesAtLine[curLine] = previousNode;
        curLine++; // THIS PART ONLY WORKS FOR EMPTY LINES - IF MULTILINE NODES ARE USED THE CODE BREAKS
      } while (curLine < this.tree.nodesAtLine.length && this.tree.nodesAtLine[curLine] === node);

      var event = new EventTypes_1.DeleteNodeEvent(node.id);
      this.parsedLine.events.push(event);
    }
  }, {
    key: "addNewNode",
    value: function addNewNode(text) {
      var _a, _b;

      log.verbose('Adding new node with text %s', text);
      var parsedLine = this.parsedLine;
      var existingNode = this.getExistingNode();
      log.verbose('Number of tree lines (%i) with cur line (%i)', this.tree.nodesAtLine.length, this.parsedLine.line);

      if (existingNode) {
        log.verbose('Node at line already exists with text %s', text); // edit existing node

        if (existingNode.headText && existingNode.headText === text) {// text is same - do nothing
        } else {
          // fire edit
          existingNode.headText = text;
          var event = new EventTypes_1.EditNodeEvent(existingNode.id, (_a = existingNode.parent) === null || _a === void 0 ? void 0 : _a.id, text);
          parsedLine.events.push(event);
        }
      } else {
        // add new node
        var node = this.tree.addNewParserNode(parsedLine.line, parsedLine.indentation, text);

        var _event = new EventTypes_1.AddNodeEvent(node.id, (_b = node.parent) === null || _b === void 0 ? void 0 : _b.id, text);

        parsedLine.events.push(_event);
      }
    }
  }, {
    key: "changeNodeIndentation",
    value: function changeNodeIndentation(node, newIndent) {
      var _a, _b;
      ...
      if (node.indentation < newIndent) {
        ...
        if (node.predecessor) {
          if (node.predecessor.indentation === node.indentation) {
            ...
            var event = new EventTypes_1.EditNodeEvent(node.id, (_a = node.parent) === null || _a === void 0 ? void 0 : _a.id, node.headText);
            this.parsedLine.events.push(event);
          } else {
        ...
        var _event2 = new EventTypes_1.EditNodeEvent(node.id, (_b = node.parent) === null || _b === void 0 ? void 0 : _b.id, node.headText);

        this.parsedLine.events.push(_event2);
      }
    }
  }, {
    key: "createNewParsedLine",
    value: function createNewParsedLine(line, content) {
      return {
        line: line,
        indentation: 0,
        content: content,
    ...
    key: "handleNotStarted",
    value: function handleNotStarted() {
...
},{"./logger.parser":"core/parser/logger.parser.ts","../events/EventTypes":"core/events/EventTypes.ts"}],"core/parser/tree/ParserNode.ts":[function(require,module,exports) {
"use strict";
...
var LexemType_1 = require("../lexer/LexemType");

var logger_parser_1 = require("../logger.parser");

var EventTypes_1 = require("../../events/EventTypes");

var log = logger_parser_1.parserLogger('node');
/**
 * The parser node is responsible for one block of the text editor.
 */

var ParserNode = /*#__PURE__*/function () {
  /**
   ...
   * @param text
   */
  function ParserNode(id, tree, indentation, text) {
  ...
  _createClass(ParserNode, [{
    key: "handleLineChange",
    value: function handleLineChange(tokens, lineParser) {
      var previousNode = lineParser.getPreviousNode();
    ...
    key: "handleHeadChange",
    value: function handleHeadChange(tokens, lineParser) {
      var token = tokens.shift();
      var lineIndent = lineParser.indentation;

      if ((token === null || token === void 0 ? void 0 : token.type) === LexemType_1.LEXEM_EMPTY_LINE) {
        // delete node
        ...
        if ((token === null || token === void 0 ? void 0 : token.type) === LexemType_1.LEXEM_NODE_TEXT) {
    ...
    value: function handleBodyChange(tokens, lineParser) {
      var _a;
      ...
      } else if ((token === null || token === void 0 ? void 0 : token.type) === LexemType_1.LEXEM_NODE_TEXT) {
        // TODO test if multiline flag is set
        log.debug('Adding new node to existing node');
        var node = this.tree.addNewParserNode(lineParser.line, lineParser.indentation, token.content);
        var event = new EventTypes_1.AddNodeEvent(node.id, (_a = node.parent) === null || _a === void 0 ? void 0 : _a.id, token.content);
        lineParser.pushEvent(event);
      } else {
        log.warn('Unhandled token: ', token === null || token === void 0 ? void 0 : token.type, token === null || token === void 0 ? void 0 : token.content);
      }
    }
  }]);
...
},{"../lexer/LexemType":"core/parser/lexer/LexemType.ts","../logger.parser":"core/parser/logger.parser.ts","../../events/EventTypes":"core/events/EventTypes.ts"}],"core/parser/tree/ParserTree.ts":[function(require,module,exports) {
"use strict";
...
var ParserNode_1 = require("./ParserNode");

var logger_parser_1 = require("../logger.parser");

var log = logger_parser_1.parserLogger('tree');
/**
 * Manages the insertion and deletion of nodes and lines.
 */

var ParserTree = /*#__PURE__*/function () {
  function ParserTree() {
    _classCallCheck(this, ParserTree);

    this.lastId = ParserTree.INITIAL_NODE_ID;
    this._nodes = {};
    /**
  ...
  _createClass(ParserTree, [{
    key: "getNodeOfLine",
    value: function getNodeOfLine(lineNumber) {
    ...
    key: "addNewParserNode",
    value: function addNewParserNode(line, indentation, text) {
      var id = this.generateNodeId();
      log.debug('Adding new parse node to line %i (%s)', line, text);
      var existing = this.getNodeOfLine(line);
      var previousNode = this.getNodeOfLine(line - 1);
      var newNode = new ParserNode_1.ParserNode(id, this, indentation, text);

      if (existing && existing !== previousNode) {
        log.error('Existing (%i, %s) equals previous (%i, %s)', existing === null || existing === void 0 ? void 0 : existing.id, existing === null || existing === void 0 ? void 0 : existing.headText, previousNode === null || previousNode === void 0 ? void 0 : previousNode.id, previousNode === null || previousNode === void 0 ? void 0 : previousNode.headText);
      ...
      if (!previousNode || previousNode.indentation === indentation) {
        ...
        newNode.successor = previousNode === null || previousNode === void 0 ? void 0 : previousNode.successor;
        ...
        newNode.parent = previousNode === null || previousNode === void 0 ? void 0 : previousNode.parent;
        log.debug('Added as sibling');
      } else if (previousNode.indentation < indentation) {
        // add a child
        newNode.parent = previousNode;
        var successor = this.getNodeOfLine(line + 1);
        ...
        if (predecessor) {
          ...
          if (predecessor.indentation !== indentation) {
            log.warn('Moved up node but sibling has not exactly the same indentation.' + 'Predecessor has %i, newNode has %i', predecessor.indentation, newNode.indentation);
          ...
          newNode.predecessor = predecessor; // update successor

          var _successor = this.getNodeOfLine(line + 1);

          if (_successor) {
            if (_successor.indentation > newNode.indentation) {
              // successor is a child element
              // => we divided two childs, update parents accordingly
              previousNode.successor = undefined;
              _successor.predecessor = undefined;
              _successor.parent = newNode;
    ...
    key: "insertLine",
    value: function insertLine(line) {
      log.debug("Inserting line at ".concat(line)); // TODO use push for end?
    ...
    key: "prevIndent",
    value: function prevIndent(lineNumber) {
      ...
      var entry = this._nodesAtLine[lineNumber];
    ...
    key: "applyLine",
    value: function applyLine(line) {
      var _a, _b;

      var curNode = this.getNodeOfLine(line);
      this._nodesAtLine[line] = curNode === undefined ? this.getNodeOfLine(line - 1) : curNode;
      log.debug('Applied node %i (%s) to line %i', (_a = this._nodesAtLine[line]) === null || _a === void 0 ? void 0 : _a.id, (_b = this._nodesAtLine[line]) === null || _b === void 0 ? void 0 : _b.headText, line);
    }
  }, {
    key: "nodesAtLine",
    get: function get() {
...
exports.ParserTree = ParserTree;
ParserTree.INITIAL_NODE_ID = 1;
},{"./ParserNode":"core/parser/tree/ParserNode.ts","../logger.parser":"core/parser/logger.parser.ts"}],"core/parser/MindNotesParser.ts":[function(require,module,exports) {
"use strict";
...
var MindNotesLexer_1 = require("./lexer/MindNotesLexer");

var LineParser_1 = require("./LineParser");

var ParserTree_1 = require("./tree/ParserTree");

var logger_parser_1 = require("./logger.parser");

var log = logger_parser_1.parserLogger('parser');

var MindNotesParser = /*#__PURE__*/function () {
  function MindNotesParser() {
    _classCallCheck(this, MindNotesParser);

    this.lexer = new MindNotesLexer_1.MindNotesLexer();
    this.tree = new ParserTree_1.ParserTree();
    this.textLines = [];
    this.lineParser = new LineParser_1.LineParser(this.tree);
  }

  _createClass(MindNotesParser, [{
    key: "parse",
    value: function parse(text) {
      ...
      this.refreshText(text); // separate lines

      log.verbose('Parsing %i lines', this.textLines.length);
      this.textLines.forEach(function (line, index) {
        _this.lineParser.start(index, line);

        _this.parseLine(_this.lineParser);

        var lineResult = _this.lineParser.finish();

        log.verbose('Got line result: ', lineResult); // TODO optimize copying
    ...
    key: "edit",
    value: function edit(startLine, startCharacter, endLine, endCharacter, newText) {
      var result = {
        tree: this.tree,
        parsedLines: [],
        events: [],
      ...
      var textChangeLines = newText.split(/\r?\n/);
      log.debug('Changing %i lines.', textChangeLines.length);
      log.debug('Split:', JSON.stringify(textChangeLines));
      var affectedExistingLines = endLine - startLine + 1; // const remainingChangedLines = textChangeLines.length - affectedExistingLines;

      log.debug('Change affects %i existing lines.', affectedExistingLines); // log.verbose('Remaining changed lines = %i.', remainingChangedLines);

      var exLineIndex = -1;
      var index = 0;

      for (; index < textChangeLines.length && exLineIndex < endLine; index++) {
        var newLine = textChangeLines[index];
        log.debug('New line "%s"', newLine);
        exLineIndex = index + startLine;
        log.debug('exLineIndex %i', exLineIndex);
        var curStartChar = undefined;
        var curEndChar = undefined;
        ...
        this.textLines[exLineIndex] = this.replaceString(this.textLines[exLineIndex] || '', curStartChar, curEndChar, newLine); // reparse changed line
        ...
        var lineResult = this.lineParser.finish();
      ...
      if (textChangeLines.length < affectedExistingLines) {
        ...
        log.debug('Removing %i lines that were squashed by the change', removedLines); // explicitly remove bottom lines

        for (var i = 0; i < removedLines; i++) {
          log.debug('Deleting internal line %i', index + startLine + i);
          this.lineParser.start(index + startLine, '');

          var _lineResult = this.lineParser.deleteLine();

          this.lineParser.finish();
          this.combineParserResult(result, _lineResult);
        }
      }
      ...
      if (textChangeLines.length > affectedExistingLines) {
        ...
        for (var _i = index; _i < textChangeLines.length; _i++) {
          this.textLines.splice(startLine + index + _i - 1, 0, textChangeLines[_i]);
          this.lineParser.start(startLine + index + _i - 1, textChangeLines[_i]);

          var _lineResult2 = this.lineParser.insertLine(); // this.lineParser.start(startLine + index + i - 1, this.textLines[startLine + index + i]);


          this.parseLine(this.lineParser);
          this.lineParser.finish();
          this.combineParserResult(result, _lineResult2);
        }
      }
    ...
    key: "combineParserResult",
    value: function combineParserResult(parserResult, lineResult) {
      log.debug('Got line result: ', lineResult); // TODO optimize copying
    ...
    key: "replaceString",
    value: function replaceString(input, start, end, newText) {
    ...
    key: "parseLine",
    value: function parseLine(lineParser) {
      var content = lineParser.lineContent; // extract line tokens

      var lexerResult = this.lexer.tokenizeLine(content);
      var tokens = lexerResult.lexems;
      ...
      if ((token === null || token === void 0 ? void 0 : token.type) === LexemType_1.LEXEM_INDENT) {
      ...
      lineParser.indentation = indent; // extract previous state

      var previousNode = lineParser.getPreviousNode();
      var existingNode = lineParser.getExistingNode(); // line content

      if (!previousNode && !existingNode && (token === null || token === void 0 ? void 0 : token.type) !== LexemType_1.LEXEM_EMPTY_LINE) {
        log.debug('This is the root node - no previous node'); // no active node - this is the first root node
    ...
    key: "refreshText",
    value: function refreshText(text) {
      this.textLines = text.split(/\r?\n/);
    }
  }]);

  return MindNotesParser;
}();

exports.MindNotesParser = MindNotesParser;
},{"./lexer/LexemType":"core/parser/lexer/LexemType.ts","./lexer/MindNotesLexer":"core/parser/lexer/MindNotesLexer.ts","./LineParser":"core/parser/LineParser.ts","./tree/ParserTree":"core/parser/tree/ParserTree.ts","./logger.parser":"core/parser/logger.parser.ts"}],"editor/TextEditor.ts":[function(require,module,exports) {
"use strict";
...
var MindNotesParser_1 = require("../core/parser/MindNotesParser");

var log = logger_1.editorLogger('editor');
document.addEventListener('selectionchange', function () {
  console.log('on selection change');
});

var TextEditor = /*#__PURE__*/function () {
  function TextEditor(events) {
    _classCallCheck(this, TextEditor);

    this.parser = new MindNotesParser_1.MindNotesParser();
    this.events = events;
  ...
  _createClass(TextEditor, [{
    ...
    value: function init() {
      ...
      area.addEventListener('change', function (change) {
        log.trace('selection change');
        console.log(change);
        log.debug('selectionStart %i, selectionEnd %i', area.selectionStart, area.selectionEnd);
      }, false); // document.onselectionchange = () => {
      //     console.log('selection change: ', document.getSelection());
      // }

      area.addEventListener('input', function (event) {
        log.trace(event);
        log.debug('target %s, selectionStart %i, selectionEnd %i , rows %i', event.data, event.target.selectionStart, event.target.selectionEnd, event.target.rows); // debounce complete reparsing (setTimeout in new thread)

        _this.parser.parse(area.value).events.forEach(function (event) {
...
},{"../logger":"logger.ts","../core/parser/MindNotesParser":"core/parser/MindNotesParser.ts"}],"index.ts":[function(require,module,exports) {
"use strict";
...
var EventHandler_1 = require("./core/events/EventHandler");

var EventRenderer_1 = require("./renderer/EventRenderer");

var TextEditor_1 = require("./editor/TextEditor"); // import Split = require("split.js");
...
(function () {
  var emitter = new EventHandler_1.SerializedEventEmitter();
  new EventRenderer_1.EventRenderer(emitter);
  new TextEditor_1.TextEditor(emitter);
})();
},{"./core/events/EventHandler":"core/events/EventHandler.ts","./renderer/EventRenderer":"renderer/EventRenderer.ts","./editor/TextEditor":"editor/TextEditor.ts"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
